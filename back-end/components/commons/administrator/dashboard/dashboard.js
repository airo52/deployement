...
class DashBoard{
   ...
   async getAssignedOrders(){
          const result = await db.read('tbl_task',['COUNT(*)'],[`WHERE tbl_task.task_status='in-progress' AND tbl_task.task_writer !='none'`]);
          var count =result[0]['COUNT(*)'];
         
          const points = await db.read('tbl_task',["COUNT(*)"],[`WHERE tbl_task.task_status='in-progress' AND tbl_task.task_writer !='none' GROUP BY DATE_FORMAT(taskl_accepted_date,'%m')`]);
            const data =[];
   ...
   async getOrdersBeingredone(){
       const result = await db.read('tbl_task',['COUNT(*)'],[`
           WHERE tbl_task.task_status='revision' AND tbl_task.task_writer !='none'
       `])
       var count =result[0]['COUNT(*)'];
      
       const points = await db.read('tbl_task',['COUNT(*)'],[`
       WHERE tbl_task.task_status='revision' AND tbl_task.task_writer !='none' GROUP BY DATE_FORMAT(taskl_accepted_date,'%m')
       `])
   ...
   async getCancelledOrders(){
    const result = await db.read('tbl_task',['COUNT(*)'],[`
    WHERE tbl_task.task_status='cancelled' AND tbl_task.task_writer !='none'
   `])
   var count =result[0]['COUNT(*)'];

   const points = await db.read('tbl_task',['COUNT(*)'],[`
   WHERE tbl_task.task_status='cancelled' AND tbl_task.task_writer !='none' GROUP BY DATE_FORMAT(taskl_accepted_date,'%m')
   `])
...
this.state.cancelledOrders.push({count,points:points === "empty"?0:data});
   ...
   async getCompletedOrders(){
    const result = await db.read('tbl_task',['COUNT(*)'],[`
    WHERE tbl_task.task_status='completed' AND tbl_task.task_writer !='none'
   `])
   var count =result[0]['COUNT(*)'];

   const points = await db.read('tbl_task',['COUNT(*)'],[`
   WHERE tbl_task.task_status='completed' AND tbl_task.task_writer !='none' GROUP BY DATE_FORMAT(taskl_accepted_date,'%m')
   `])
...
this.state.completedOrders.push({count,points:points === "empty"?0:data});
   ...
   async getpendingReviews(){
    const result = await db.read('tbl_task',['COUNT(*)'],[`
    WHERE tbl_task.task_status='pending' AND tbl_task.task_writer !='none'
   `])
   var count =result[0]['COUNT(*)'];

   const points = await db.read('tbl_task',['COUNT(*)'],[`
   WHERE tbl_task.task_status='pending' AND tbl_task.task_writer !='none' GROUP BY DATE_FORMAT(taskl_accepted_date,'%m')
   `])
